function [best_action, max_reward] = planner1(robot_pos, rem_budget, bel_space, MapParameters)
%calculates the best action to take within the robot's action space

%get action space
action_space = getActionSpace(robot_pos, rem_budget, MapParameters);
reward_action = zeros(size(action_space,1),1);

%iterate through actions to find reward
for i = 1:size(action_space,1)
    reward_action(i) = getReward_1(action_space(i,:), rem_budget, bel_space, MapParameters);
end
